...
void ElevatorInspection(int elevatorNumber, bool inspecting);
...
int main()
{
  ...
  for (int i = 0; i < 6; i++)
  {
    status[i].insinspection = true;
    status[i].floor = 1;
    status[i].Min = 100;
    status[i].Max = 0;
    _beginthreadex(NULL, 0, thread_el1, i, 0, NULL);
  }
  _beginthreadex(NULL, 0, UI, 0, 0, NULL);
        ...
        else if (menu == 2)
        {
          ...
          if (inspection >= 1 && inspection <= 6) { // 사용자가 입력한 엘리베이터를 점검
            if (status[inspection - 1].insinspection == false) {
              ElevatorInspection(inspection - 1, true);
            }
            else {
  ...
  while (1)
  {
    if (status[n].status > 0) {
      if (status[n].status == 1) {
        for (int i = 0; i < 100; i++) {
          people = elevator[i].people;
          if (elevator[i].elevator_num == n) {
            if (elevator[i].button == 1) {
              if (status[n].floor == elevator[i].currentFloor && status[n].full == false) {
                for (int i = 0; i < people; i++) {
                  if (status[n].totalweight + elevator[i].weight.man * 75 + elevator[i].weight.woman * 55 > 1450) {
                    gender = rand() % 2;
                    if (gender == 0)
                    {
                      if (elevator[i].weight.man == 0) {
                ...
                status[n].totalweight = status[n].totalweight+elevator[i].weight.woman * 55 + elevator[i].weight.man * 75;
                status[n].totalpeople = status[n].totalpeople+elevator[i].people;
                status[n].Doors = true;
                /*DrawElevator(0);
                DrawElevator(1);*/
                if (usertype == 'u')
          ...
          if (elevator[i].elevator_num == n) {
            if (elevator[i].button == 2) {
              if (status[n].floor == elevator[i].currentFloor) {
                if (status[n].totalweight + 55 > 1450) {
                  status[n].full = true;
                }
                status[n].totalweight = status[n].totalweight+elevator[i].weight.woman * 55 + elevator[i].weight.man * 75;
                status[n].totalpeople = status[n].totalpeople+elevator[i].people;
...
{
  int diff[6];
  int Min = 100; // 최소값을 찾기 위해 초기값을 최대값으로 설정
  int el = 0;
  ...
  for (int i = 0; i < 6; i++)
  {
    if (status[i].status == 0)
      ...
      diff[i] = 100; // 다른 상태의 엘리베이터는 선택되지 않도록 최대값 설정
...
{
  // 엘리베이터 정보 초기화
  status[elevatorNumber].insinspection = inspecting;
  ...
  // 엘리베이터에 타 있던 사람들을 내리도록 처리
  for (int i = 0; i < 100; i++)
  {
    if (elevator[i].elevator_num == elevatorNumber)
    {
      // 점검 중인 엘리베이터에 있던 승객은 현재층에서 내린다고 가정
      if (elevator[i].currentFloor != 0)
      {
        status[elevatorNumber].totalweight -= elevator[i].weight.woman * 55 + elevator[i].weight.man * 75;
        status[elevatorNumber].totalpeople -= elevator[i].people;
